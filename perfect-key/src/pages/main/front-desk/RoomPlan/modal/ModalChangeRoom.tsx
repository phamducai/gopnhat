...
import { Select, Modal, Divider, Table, Input } from 'antd';
...
const ModalChangeRoom = ({ isVisible, setIsVisble, getData, searchAndMapData, dataSelected }: Props) => {
    ...
    const fetchRooms = async (roomTypeGuid: string | undefined) => {
        if (roomTypeGuid) {
            ...
            if (dataSelected) {
                arrivalDate = dataSelected[0].arrivalDate.toString();
                departureDate = dataSelected[0].departureDate.toString();
            }
    ...
    const pushAssignData = (guest: SelectedGuestProfile, room: RoomInfo) => {
        const data: AssignRoom[] = [...assignData];
    ...
    return (
        <CModel
            title={t("FRONTDESK.ROOMPLAN.changeRoom")}
            visible={isVisible}
            onOk={() => handelOk()}
            onCancel={() => {
                setIsVisble(false);
                            ...
                            <Table
                                ...
                                rowSelection={{
                                    ...
                                    onSelect: (record: RoomInfo, selected: boolean, selectedRows: RoomInfo[], nativeEvent: Event) => {
                                        if (selected && selectedRows.length <= dataSelected.length) {
                                            dataSelected.forEach((guest: SelectedGuestProfile, guestIndex: number) => {
...
export default React.memo(ModalChangeRoom);