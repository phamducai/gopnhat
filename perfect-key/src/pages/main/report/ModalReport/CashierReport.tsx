...
import { ICashierDetailReport } from 'common/report/define-report';
...
const { Option } = Select;
interface PropsCashierReportModal {
    isShowModal: boolean;
    setIsShowModal: React.Dispatch<React.SetStateAction<boolean>>,
    title: string,
    isLoadingPrint: boolean,
    onSubmit(data: ICashierDetailReport): void
}
export const CashierReportModal = ({ setIsShowModal, isShowModal, title, onSubmit, isLoadingPrint }: PropsCashierReportModal): JSX.Element => {
    ...
    const handleOk = handleSubmit(async (data: ICashierDetailReport) => {
        onSubmit(data)
    })
                    ...
                    <form id="formCashierReport" onSubmit={handleOk} className="custom-scrollbar-pkm">
                        <div className="grid grid-cols-12 gap-4">
                            ...
                            <div className={`col-span-12 `}>
                                    ...
                                    <Radio.Group
                                        ...
                                        onChange={e => {
                                            ...
                                            if (e.target.value === DateTypeStatus.Today) {
                                                ...
                                                setValue("dateSchedule", [new Date(), new Date()])
                                                ...
                                                setValue("dateSchedule", [addDays(new Date(), 1), addDays(new Date(), 1)])
                                                setValue("time", [addDays(new Date(), 1), addDays(new Date(), 1)])
                                ...
                                name="dateType" defaultValue={0} control={control} />
                            ...
                            <div className="col-span-6">
                                ...
                                <Controller
                                    ...
                                    name="groupUser" control={control} defaultValue={''} />
                            </div>
                            <div className="col-span-6">
                                <label className="m-0 font-base font-bold">
                                    {t("REPORT.userName")}
                                ...
                                <Controller
                                    render={({ onChange, value, ref }) =>
                                        <Select className={`${classes.selectBackground} w-full !rounded-md`} placeholder={t("BOOKING.RESERVATION.EDITRESERVATION.selectHere")}
                                            ...
                                            onChange={(e) => { onChange(e) }}
                                        >
                                            <Option key={0} value={value}>{userName}</Option>
                                            
                                        </Select>
                                    }
                                    name="username" control={control} defaultValue={userName}/>
                            </div>
                        </div>
                    </form>
                </>
