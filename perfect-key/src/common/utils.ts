 ...
 */
    static parseLocaleNumber(stringNumber: string) {
        const thousandSeparator = Intl.NumberFormat("en-US").format(11111).replace(/\p{Number}/gu, '');
        const decimalSeparator = Intl.NumberFormat("en-US").format(1.1).replace(/\p{Number}/gu, '');
